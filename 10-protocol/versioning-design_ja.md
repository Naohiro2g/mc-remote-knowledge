...
`alpha`はtag前のGitHub source commitを動かすchannelであり、`aN` releaseを予約・採番しない。exposure（public / lan-only / isolated）はchannelと別軸で、alphaから公開範囲を暗黙導出しない（DECISIONS `2026-07-23-01`）。配備時は実験対象componentだけを選択時のexact source commitからbuildし、repository、commit SHA、build recipe、toolchain、artifact hash、build provenanceをlockへ記録する。他のcomponentは既知artifactへ固定し、公開artifactの版を増やさない。alphaで価値あるsliceが確認できたら次の`bN`をtagしてbeta環境へ昇格する。tagなしsnapshotを`b2+`等の擬似版でbetaへ置かず、betaを小刻みに出す。
