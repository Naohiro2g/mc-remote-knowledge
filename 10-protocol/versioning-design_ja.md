...
各段は「次に広い聴衆をテストに招く」こと。**招く相手は、その相手自身のチャンネルで配る**。聴衆もチャンネルも段ごとに単調に広がる。**境界の本体は段名ではなく遷移ゲート**であり、「段に入る条件（内容イベント）」と「段を出る条件（公開イベント）」は別の遷移＝**段境界の上下の縁**に割り当てる（1ラベルに二軸を overload しない）。
...
| `bN` | OSS開発者・貢献者 | **bootstrap**: GitHub(pre) / GitHub(pre)　**mature**: Modrinth(beta ch) / PyPI(pre) | 公開previewにできるsliceが動く |
| `rcN` | 初学者（希望者）・広く | **bootstrap**: GitHub(pre) / GitHub(pre)　**mature**: Modrinth(beta ch) / PyPI(pre) | **API 凍結**（＝教材を書いてよい合図） |
| `N`（stable） | 全員（既定） | Modrinth / PyPI | 全コンポーネント mature ＋ soak（§10.9 AND 律速） |

### 10.3 遷移ゲート（境界の本体）

- **tag前→b ＝ 公開previewにできるsliceのtag。** 大きなbetaを待たず、価値のある単位で`b1` / `b2` / `b3`…を切る。
- **b→rc ＝ API 凍結。** 凍ったら初学者に晒せる＝**教材執筆 OK の合図**。`20-教材` は rc タグを green light とし、動く API に対して教材を書かない（教材は rc soak 中に書ける）。
...
正準手順は **`uv add "minecraft-remote-api==<exact>"`**（exact-pin）。素の `pip`/`uv add`（無指定）は pre-release を既定でスキップして最新 stable を拾うため、**beta / rcを公開チャンネルに置いても既定経路では誰も踏まない**。`--pre` は教えない。生徒は常に stable をexact-pin、beta / rcテスターも対象版をexact-pinする。`pip`自体も正準手順では非推奨（uv統一 `2026-06-24-02`）。
...
**mode 遷移ゲート（bootstrap→mature）**：①公開チャンネルへのpublishが再現可能（手作業一発勝負でなく手順書／`gh` / `uv publish`で反復可能）②soak / yankの運用を最低1サイクル通した ③退避手順を確認済み（PyPI yank・Modrinth channel変更/版削除で戻せる）④**exact-pin runbookを対象者で検証済み**（OSS開発者のbeta opt-inと初学者希望者のrc opt-inを混同しない）。
