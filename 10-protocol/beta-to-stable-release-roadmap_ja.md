...
`b7-integrated-source-set-1`として固定する。同日、coordinatorがisolated checkoutからJAR、wheel／sdist、GUI、common
WireScope ZIP／manifestを自ら再生成し、`b7-integrated-artifact-input-1`として記録したが、これはcomponent ownerが
生成・返却したartifactではなく役割境界を越えていたため、正式gate入力から失効させた。bytesとtest観測は履歴比較用に残し、
release provenanceへ使わない。次はScratch担当が`develop@773e298…`からGUI、Bridge、common WireScopeを独立生成し、
coordinatorは返却identityを再buildせず横断照合する。指示書は
[`b7 Scratch owner artifact生成指示書`](../13-scratch-client/b7-owner-artifact-generation-instructions_ja.md)、
