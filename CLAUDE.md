...
- 現在の SSOT を最優先にしつつ、SSOT 自体を改善の対象として扱う。過去の決定を carry-in / 改訂 / 参照する場面で（archive 限定ではなく現行の確定行も同じ）、その決定が今も最適かを一度問い、問うたことを判断履歴に残す。過去の決定へ重みの札を貼らず、**いま下そうとしている決定が重いか**を確定の場で判定して吟味の深さを決める。重い決定には有効な局面と、局面が変わったとき何を失うかを書く（期日ではなく局面）。契機を待たず領域を丸ごと再吟味する随意棚卸しも正式な手段とする。両者の緊張は本質なので統一規則で解消せず、その場で突き合わせる（正本は README「最上位の動作原理」§メタ判断、決定 `2026-07-25-05`）。
- 不要な禁止ルールを作らない。誤りと判明したルールは履歴を消さずに撤回／改訂先を示し、現行の説明・手順・検査から除く（正本は README「最上位の動作原理」§メタ判断、決定 `2026-09-04-05`）。
- 疑う側だけでなく跳ぶ側も置く。根拠が薄くても言語化が足りなくても確定させるべきことがあり、跳躍に反証条件を要求しない。跳んだ判断は根拠が薄いことを隠さず書く。書けない直感を理由に落とさない。速度優先で決定と改訂が積み上がり認知容量を超えるとき、進行を止めて mind-size へ分解し直すのは遅延でなく正当な手段（正本は README「最上位の動作原理」§跳躍、決定 `2026-07-25-06`）。
