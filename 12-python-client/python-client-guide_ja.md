...
> **⚠ 失効**：以下の説明は旧設計のものです。現行（`2026-08-02-05`）では、
> ①ヘッダーの `PLATFORM` / カテゴリ別代替版の内訳は **projection manifest**（別ファイル）へ移り、
> ②`block` / `entity` / `particle` は接続先サーバーから**一括で取得**されるため
> 「entity だけ少し前のバージョンで代替」は起きず、
> ③このファイルは **Git に含めません**（`.gitignore` します）。
> 「いまどのバージョンか」は diff ではなく manifest の `catalogHash` で見ます。
