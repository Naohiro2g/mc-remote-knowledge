...
- shared環境（`dev-integration`等）への接続先（host／IP／port）はSSOTに存在しない。物理hostのSSH aliasだけは
  `release-operations-responsibility-design_ja.md` §3で公開参照するが、実IPはprivate（backstage所有）である。
  component担当はSSOTを探し続けず、backstageへも直接アクセスせず、gate coordinatorが指示票／確認票へ含めて
  渡す接続先を使う（`2026-09-03-03`）。coordinatorから接続先が渡されていない場合はcoordinatorへ確認する。
...
- 却下案は「その時点で採らなかった案」の記録であり、現行の禁止規則ではない。DECISIONS や他リポの
  却下案を根拠に自リポのルール・test・validation を作らない。拘束が必要なものは決定本文に肯定形で
  書かれている。搬送票の却下案は 3 件までとし、4 件以上書きたくなったら決定が大きすぎる兆候として
  分割を検討する（`2026-09-07-02`）。
...
- **粗い現在地（保存）**: grand-design-roadmap の `## 現在地` に R1／R2／R3 の成熟段階を簡潔に記す。相を見直すときは理由を DECISIONS に残し、知識リポで反映する。dev リポの参照用 clone は read-only とし、そこで更新せず知識リポへ搬送する（`2026-07-19-04`、`2026-09-26-01`）。
