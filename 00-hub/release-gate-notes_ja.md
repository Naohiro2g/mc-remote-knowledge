...
- current phase: exact compatibility set凍結済み／Stack preset・bootstrap contract投影のmerge承認・merge待ち
...
- authorized next action:
  ...
  - Stack: draft PR `#27` head `a01528a1066680393cc1280ce2d96e6a18dfee6f`はcoordinator review済み。draftを解除して同headをmergeし、merge commitとremote main一致を返す。merge後のbranch／worktree cleanupは任意だが、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deploy、live試験は次のcoordinator許可まで開始しない
