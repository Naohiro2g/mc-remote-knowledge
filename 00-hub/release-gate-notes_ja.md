...
- environment preparation: Stack PR `#26`はhead `62bac9059b991e2e6a86f6aea1e077490a538888`、draft解除済み、GitHub上でmergeable／clean。current self-verifying lockの一つの`git-build` artifactに限り、review指定SHA／lock SHA／実bytesを照合してCASへatomic create-if-absentする`mcrctl artifact import-reviewed`を実装。既存entry再hash、symlink／非regular file／容量拒否、改ざん時上書き禁止、build／download／render／apply非実行をknowledge coordinatorが差分review済み。標準port `25565`／`25575`、別portへの自動退避禁止、未知listener時のreadiness取消とbackstage照合も差分review済み。担当報告は関連26 tests、全pytest、ruff、diff check PASS。GitHub review／CI checkは未設定。runbook内のhost install許可境界とMcRemote artifact取得入力に下記二差分が残るため準備完了とはしない
- input correction: McRemote作業票のknowledge SHA `f50ebb17…`は存在せず、実在する`f50ebb13f00facfc2e73163a24f002f4c8b77d43`を参照。契約差分なし
- target deployment / profile / lock: 通常dev環境`dev-integration`／`home-server@5`候補／lock未生成。physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`。先に同portで稼働していたMinecraft runtimeは停止済み。backstage PR `#3`は修正head `7bffaa1bc1059975375904eccae8a3e2f5b4ef18`／merge commit `b0dc27174860b723ca8e19360162d6de53ee57fb`でmerge済みで、観測時点の未知service／listenerなし。公開Stack PR `#26`の現head `62bac9059b991e2e6a86f6aea1e077490a538888`も標準portと完全inventory規律へ一致。LAN到達性とfirewallは未確認。order／lock、bootstrap contract、artifact配置、candidate deployは未実施。ケータリング型は本gate対象外
- authorized next action:
  ...
  - Stack: PR `#26`で、exact set未凍結中はread-only preflightまでという停止点に合わせ、`bootstrap-ubuntu-operator.sh --install`をcoordinatorの明示許可前に実行できる説明を除く。またMcRemote exact入力をcredential-free HTTPSだけに限定せず、`git-build` provenance＋review済みbytes importを選べる記述へ揃える。回帰test後の更新headを返し、公開review／通常mergeへ進める。実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployはまだ行わない
