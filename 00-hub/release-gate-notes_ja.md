...
- current phase: **最終artifact candidate固定／公開承認待ち**。`b6-integrated-source-set-1`から六artifactを再生成・durable stagingへ固定し、McRemote統合JARのhost-native通常dev smokeをPASSした。人間承認後のmanual workflowでScratch／Bridge multi-arch OCIをGHCRへpushし、index／platform／attestation identityを照合した。`b6-artifact-candidate-set-4`を最終candidateとして受理し、tag／GitHub prereleaseはまだ作成していない
...
- authorized next action: human release ownerへ`b6-artifact-candidate-set-4`の公開承認を求める。承認後、McRemote `v1.21.11-2300.0.0b6`、Python `v2300.0.0b6`、Scratch `v2300.0.0b6`を上記default branch SHAへ固定し、prerelease ON、draft OFF、Latest非対象でGitHub releaseを作成する。McRemoteは統合JARをassetへ登録し、Pythonはwheel／sdist、ScratchはOCI／GUI／WireScope identityをrelease notesへ記載する。作成後にtag target、公開状態、asset／notes digestをAPIで再確認する。PyPI、Modrinth、public server deploy、通常dev再構築は含めない
