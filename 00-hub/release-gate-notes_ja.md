...
- current phase: exact compatibility set凍結済み／通常dev runbook merge承認・pre-apply待ち
...
- exact compatibility set / freeze status: **凍結**。下記McRemote、Scratch／Bridge、WireScope、Python、runtime／Paper／Java、runtime policy／schemaを一組とする。いずれかのcommit、artifact bytes、schema、policyが変わればこのsetを失効させ、coordinatorへ戻す
...
- environment preparation: Stack PR `#26`はreview済みhead `ba9153bd9210e66a1732dfdc55cd589f567bf2ee`／merge commit `ddfb92a1dca108c906f31e670057681e0855dfef`でmerge済み。current self-verifying lockの一つの`git-build` artifactに限り、review指定SHA／lock SHA／実bytesを照合してCASへatomic create-if-absentする`mcrctl artifact import-reviewed`を実装。既存entry再hash、symlink／非regular file／容量拒否、改ざん時上書き禁止、build／download／render／apply非実行をknowledge coordinatorが差分review済み。標準port `25565`／`25575`、別portへの自動退避禁止、未知listener時のreadiness取消とbackstage照合、exact set未凍結中のhost install禁止、McRemote artifactのHTTPS／review済みgit-build二経路も一致。担当報告はrunbook回帰16 tests、全pytest、ruff、diff check PASS。GitHub CI checkは未設定。通常dev profile／runbook／review済みimport経路の準備は完了。通常dev exact baseには、b4で使用済みのMinecraft `1.21.11`、Paper `1.21.11-132`（54,846,016 bytes／SHA-256 `5ffef465eeeb5f2a3c23a24419d97c51afd7dbb4923ff42df9a3f58bba1ccfba`／credential-free HTTPS）、Java floor `21`、runtime `docker.io/itzg/minecraft-server:2026.7.2-java21`（OCI index digest `sha256:7f69fd6688e03495c8a8f5a46e8a8e82001b4465f4b55bdcd024c02c3624d8c8`）を採用する。Stack merged registryの`mcremote-paper@5`／`@6`と一致することをcoordinatorが確認済み。linux/amd64 child digest `sha256:4d9821830112c814d09d7c0a39d74f0962112fd56707f337210491ce6f8cd8f7`とruntime内Java `jdk-21.0.11+10`はpresetの追加identityにせず、実pull／起動時の照合対象とする。Stack PR `#27`はreview済みhead `a01528a1066680393cc1280ce2d96e6a18dfee6f`／merge commit `6451190337a9122a7f87848c49bac84f5eb82626`でmerge済み。remote mainとmerge commitの一致、reviewed head包含をcoordinatorがGitHub APIで再確認した。`mcremote-paper@7`のsemantic SHA-256は`ce5dafda624b71a0253d5f7f5af7703a75a84c9861ba6a44a508ffc25008c77d`。凍結済みserver artifact、bootstrap tuple、旧`f293…`非包含、自己完結したtoolchain identityを維持する
- environment staging result: Stack main `6451190337a9122a7f87848c49bac84f5eb82626`でoperator bootstrap、Docker direct access、未知service／listenerなし、port `25565`／`25575`空き、workstationから一時listenerへのLAN data-plane到達、validate／plan、human EULA承認、理由付きunverified acknowledgement、artifact fetch／review済みimport、renderがPASS。UFW rule一覧のlive再読は対話sudo未実施だが、Docker導入後の実data-plane確認を現段階の根拠として受理し、apply後の実runtime／doctorで再確認する。CASはregular file 2件／symlink 0件で、Paper `5ffef465…ccfba`とMcRemote `17cdc457…aeb6`が凍結setに一致。OCI index `sha256:7f69fd…d8c8`はlockへ固定済み、local pullはapply未許可のため未実施。renderはadapter `compose@5`、lock binding、OCI／Paper／McRemote digest、EULA、auth enforcementが一致し、owner mismatch／symlink 0。apply／runtime起動／doctor／live未実施。通常dev runbookに理由付きunverified acknowledgementの具体的設定手順がなく、実作業者が会話外の判断を要したため、apply前のrunbook cleanupとする
- runbook cleanup: Stack draft PR `#28` head `94b86b6f4142024a443a77bcfb1a39c8107aace9`は、通常dev guideとrunbook testの2 filesだけを変更する。orderの既存acknowledgement 2 scalarを具体的なgate理由で設定し、再validate、`resolve --allow-unverified`、plan review、後続`apply --allow-unverified`へ進むdurable＋one-shot境界、成功条件、`acknowledgement_reason_required`／`unverified_not_acknowledged`からの戻り先を記載する。lock／generated treeの手編集は禁止したまま。coordinatorはPR全体、base `6451190337a9122a7f87848c49bac84f5eb82626`、head、DRAFT／MERGEABLE／CLEAN、外部check 0件をGitHub APIで照合した。担当報告は452 tests／ruff／diff check PASSで、preset／order／lock／artifact／render identity不変。CLI変更なし。PR `#28`はmerge可
- input correction: McRemote作業票のknowledge SHA `f50ebb17…`は存在せず、実在する`f50ebb13f00facfc2e73163a24f002f4c8b77d43`を参照。契約差分なし
- target deployment / profile / lock: 通常dev環境`dev-integration`／`home-server@5`／`mcremote-paper@7`。order semantic SHA-256 `97561f1b49aa8f4d96e59ab24647ff2fb3ef93dab670178c33dc9867f13c708d`／file SHA-256 `a996aa50b5f1e9c11057b6dcaed2915f08c99c0a775635ed305153f054d95411`、lock identity `sha256:b5840f077f0a6fd55221be1795751cad809da00a033933df3f461ca2292ab705`／file SHA-256 `5043f8225a67c20c5d397d71843eb3d5ec20e5720c1bc8b7368a0224cadeb35b`、render manifest SHA-256 `6cd55bd02b6ed52958e6ee163ab852ec809bc0c01541f571e3943e75d3778ea6`。physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`。backstage PR `#4`はreviewed head `b41a853540ab156b2eb871ecd246192ecf0940c8`／merge commit `e6802dbbb3fd6fc890c7bb37c90d663c6ce0dd05`でmerge済み。coordinatorはremote main一致とreviewed head包含をGitHub APIで確認し、private内容はknowledgeへ複製していない。candidate deployは未実施。ケータリング型は本gate対象外
- authorized next action:
  ...
  - Stack: draft PR `#28` head `94b86b6f4142024a443a77bcfb1a39c8107aace9`はcoordinator review済み。draftを解除して同headをmergeし、merge commit、remote main一致、reviewed head包含を返す。mergeまでprofile、preset、order、lock、artifact、renderを変更しない。`apply`、runtime起動、`doctor`、candidate deploy、live試験は次のcoordinator許可まで行わない
  - backstage: PR `#4`のmergeを完了。private inventoryの実値をknowledge／Stackへ複製せず、host状態が返却票から変わった場合だけreadinessを取り下げてcoordinatorへ返す
