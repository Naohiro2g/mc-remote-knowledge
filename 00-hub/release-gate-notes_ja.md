...
- current phase: **統合後pre-OCI artifact入力＋McRemote smoke固定／最終artifact gateはOPEN**。`b6-integrated-source-set-1`からBridge OCIを除く六artifactを再生成・durable stagingへ固定した。McRemote統合JARは通常devで起動・HEALTHY・auth否定パス・新規session認証済み代表call・同tokenの正常再起動後再利用までPASSした。Scratch test exact集計、Bridge OCI、release判定が残る
...
- gate manifest identity: Tier 2 evidence入力は`b6-artifact-candidate-set-3`、現行sourceは`b6-integrated-source-set-1`、Bridge OCI前の六artifactは`b6-integrated-artifact-input-1`としてそれぞれ履歴固定する。入力manifest SHA-256は`1a3b40fc3747359bd2a206f37aa4b8508989b97aedc6b6d584d8cfd49b3c4a4b`。最終`b6-artifact-candidate-set-4`はまだ成立していない。人間可読のbinding recordは`10-protocol/b6-artifact-candidate-record_ja.md` §7〜§9である
...
- authorized next action: Scratch ownerはaggregateのpassed／failed／skipped等のexact内訳を返す。現行manual image workflowは`push: true`でGHCR uploadを伴うため、Bridge OCI生成・pushは人間承認を待つ。承認後は統合Scratch SHAからmulti-arch OCIをbuild／pushし、index／platform digest、SBOM／provenance、container smokeを固定する。公開release、tag、その他registry uploadを本段階で実行しない
...
- non-claim: 観測target変更後の自動再開、live `dropped_frames`、Scratch exact aggregate、最終artifact set、Bridge exact OCI candidate、公開release artifact freeze、b6 `GREEN`／releaseを主張しない
