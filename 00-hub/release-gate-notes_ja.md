...
- current phase: **host-native通常devの最小bootstrapを許可**。Docker runtime停止からhost Java／Paper／McRemoteの直接起動・environment smokeまでを一巡し、汎用profile設計を先行させない。製品live-auto／live-humanはenvironment返却後に統一実施票で開始する
...
- authorized next action:
  ...
  - Stack: **host-native bootstrapを直ちに一巡する**。①旧Docker runtimeを正準停止し、自動再起動なし・container非稼働・port解放を確認する。旧volume、CAS、order、lock、generated tree、image、Docker installationは削除しない。②hostのJavaを確認し、major 21でなければOSの正規package経路からheadless Java 21を導入する。vendor、full version、package identity、実行binary SHA-256を固定する。③既存CASのPaper `5ffef465…ccfba`とMcRemote `17cdc457…aeb6`を再hashし、専用service account／deployment directoryへ配置する。world、credential store、revocation authorityを分離し、EULA、標準port `25565`／`25575`、auth enforcementを設定する。④最小のsystemd unitと停止／再起動／log確認手順をStackの通常dev runbookへ置き、手順どおり起動する。⑤service health、Paper／McRemote artifact、listener、workstationからのLAN到達、tokenなしhelloの`auth_required`、一回の正常再起動、未知service／listenerなしを確認する。汎用rendererやDocker doctor修正を先に作らず、失敗時は現場で手修正を重ねずreasonと最小差分を返す。製品APIのlive-auto／live-humanはまだ行わない
