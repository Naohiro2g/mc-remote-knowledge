...
- authorized next action:
  ...
  - Scratch: source `602ecdf809f87a7e33e50d7c465b7248429e26dc`と上記artifact identityを変更せず、Pythonの独立再生成照合まで待機する。candidate deploy／liveを開始しない
  - Python: Scratch `602ecdf809f87a7e33e50d7c465b7248429e26dc`をclean checkoutし、manifestのNode `24.19.0`／exact recipeでcommon WireScope artifactを再生成する。ZIP／manifestが上記両SHAへ一致した場合だけ第1段階commitへ同梱して再固定し、全決定論的test、wheel／sdist build、metadata／RECORD／license検査後のpush済みsource／artifact identityを返す。不一致時はcandidateを作らずScratchへ差分を返す
  - Stack: PR `#26`のmerge後repo-local branch／worktree cleanupだけ実施可能。component candidate収束とexact set凍結まで待機し、coordinatorの次許可までは実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployを行わない
