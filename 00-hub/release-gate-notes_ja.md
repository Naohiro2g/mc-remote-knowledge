...
- current phase: **source candidate set 2からMcRemote／Python fixture投影完了／`data.allowed`順のMcRemote追補待ちでHOLD**。両repoのfixture bytes／case接続は確認したが、McRemote productionの配列順がowner fixtureと異なるため三repo横断PASSはまだ確定しない。candidate artifact、shared環境deploy、正式evidence、default branch統合、release判定は未着手
- contract maturity / required test tier: b6 wire contract自体は固定済みで、根本仕様へ戻さない。Tier 2共有contract照合の前にScratchの局所gapをTier 0〜1で閉じる。横断`GREEN`やTier 3完了は主張しない
- knowledge contract: `10-protocol/versioning-design_ja.md` §10.11.4、`10-protocol/wire-format-design_ja.md` §5.4／§5.8／§7.3、`10-protocol/beta-to-stable-release-roadmap_ja.md` §3.1、`10-protocol/b6-compatibility-fixture-plan_ja.md`、`15-wirescope/wirescope-deployment-design_ja.md` §14。DECISIONS `2026-08-26-05`／`2026-08-26-06`／`2026-08-26-08`／`2026-08-27-01`／`2026-08-27-02`／`2026-08-27-03`。core b6 contract baselineは`c461a53a7c17caa5ecd48ac429b500595cd335e1`
- gate manifest identity: `b6-source-candidate-set-2`。source／owner fixture入力だけを指し、artifact manifestではない
- exact compatibility set / freeze status: **fixture投影の比較基準としてsource入力凍結、release進行はHOLD**。McRemote `codex/b6-protocol23-cleanup@9b8b130808d8e1d1288f038dd04f738a86177e35`、Python `codex/b6-protocol23-python@69a160aecfc6cd346b3341cdf10007e2903b5207`、Scratch `agent/b6-source-refresh@104f194deddc9c244e6e07c4223965c792551f9d`。protocol `23.0.0`、予定artifact version `2300.0.0b6`。set 1は監査履歴として保持するがrelease入力としてset 2がsupersedeし、candidate artifactはまだ生成しない。McRemote／Pythonのfixture投影でsource identityが変われば次setへ更新する
- component readiness:
  - McRemote: `56b83c0915a5e354704cd86f1bac6c7b00cf137f`でowner fixture二件をexact bytes配置し、sign／event／handleのfixture-driven test 10件を追加、全143/143 PASSを報告した。fixture digestと親SHA／変更pathはcoordinator照合済み。ただし`data.allowed`を集合比較してproductionのalphabetical順とowner順の差を許したため、`2026-08-27-03`への一commit追補待ち。live、artifact、正式evidence、releaseは未実施・未主張
  - Python: `0ba22e80b9b1b339dfd11085b1b24cef646599b2`でowner fixture二件をexact bytes配置し、`test_b6.py`を全case群へ接続、handle suffixの22文字以上制約をclient／observer両方から除いた。対象18/18、全242/242 PASS報告、fixture digestと親SHA／変更pathはcoordinator照合済み。live、artifact、正式evidence、default branch統合、releaseは未実施・未主張
  - Scratch: set 1のbrowser保存／protocol 23／`pickaxe_poke`／sign／`mcr_eh_`／WireScope系を維持したまま、set 2で`@mc-remote/protocol`へsign mirrorとowner fixtureを追加し、診断用`CLIENT_VERSION`を`2300.0.0b6`へ更新、歴史`spawn-v22.json`とcurrent `mcr_eh_` assertionを分離した。refresh申告はprotocol 22/22、対象VM 416/416、protocol lint／build、VM／GUI build、変更範囲lint PASS。plugin／Pythonとの横断互換、実plugin E2E、exact artifact、default branch統合、releaseは未主張
...
- canonical case audit: `10-protocol/b6-compatibility-fixture-plan_ja.md`。protocol／artifact identity、handle、sign、`pickaxe_poke`、legacy method不在をcase IDへ分け、set 1のgap、set 2のowner発行、McRemote／Python投影、`data.allowed`順の残差分まで記録した。根本wire shapeの再設計を要求する差分はない
- shared fixture owner: Scratch `agent/b6-source-refresh@104f194d…`の`@mc-remote/protocol/test/fixtures/`へ発行済み。`sign-v23.json`はSHA-256 `7ffb63c264602cba56117eefff1f9604b955df04c5cc655e877772b8ff7cd30e`、`events-v23.json`は`31760d267f3c2641042fbe8595fda9c259134a1c05423271a99cb74da1efa9aa`。GitHub上のexact bytesをcoordinatorが再hashし、McRemote／Pythonの配置bytesも一致確認した。case接続は完了、McRemote productionの`data.allowed`順だけが追補待ち
- authorized next action: McRemote担当だけが`56b83c0915a5e354704cd86f1bac6c7b00cf137f`を基準に、`SignCommands`の色／装飾`ALLOWED_*_TOKENS`を`sign-v23.json`の配列順へ固定し、`SignFixtureContractTest.b6S06RejectsUnknownColorAndDecorationTokens`を集合比較からlist順のexact比較へ変える。一つの新しいpush済みSHA、変更path、143件以上の全test PASS、fixture digest不変、non-claimを返す。Scratch／Python source、owner fixture bytes、`LineValue.decorations`のcanonical昇順、runtimeの受理token集合は変更しない。新SHA固定前にcandidate artifact、shared環境deploy、人間参加試験へ進まない
