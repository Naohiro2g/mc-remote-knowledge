...
- current phase: **HOLD — 通常devのhost-native topologyへ訂正中**。candidate apply、runtime起動、doctor、live-auto、live-humanは未許可
...
- target deployment / profile / lock: logical deploymentは`dev-integration`、physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`を維持する。profile、preset、host Java exact identity、service manager、order、lock、renderは**host-native／non-Docker案として未確定**。ケータリング型とDocker／Compose型はいずれも本gate対象外
- authorized next action:
  ...
  - Stack: **緊急停止だけを許可**。旧Docker runtimeを正準な停止操作で停止し、自動再起動しない状態、container状態、port `25565`／`25575`解放、停止時errorの有無を返す。volume、CAS、order、lock、generated tree、image、Docker installationは削除・変更せず、再apply、doctor修正、doctor再実行、rollback、製品試験を行わない。停止確認後、Paper／McRemoteをhost Javaで直接動かす最小のappend-only profile／preset／renderer／service manager／artifact verification／filesystem ownership／world・credential・revocation分離／rollback／doctor案と、現Stackで不足する実装を設計票として返す。実装、追加host変更、applyは新たなcoordinator許可まで行わない
  - backstage: Stackの停止後にcontainer／runtime停止、container状態、標準port解放、未知service／listenerなしをprivate inventoryと照合してsanitized確認を返す。導入済みDockerと旧managed dataは観測事実として管理し、削除・再構成しない。host-native案に必要なservice account／directory／service managerのprivate配置候補はStack設計票の入力として返せるが、作成しない
