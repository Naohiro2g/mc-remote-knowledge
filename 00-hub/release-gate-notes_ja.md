...
- current phase: McRemote component candidate再現性修正／exact compatibility set入力組立。通常dev環境の設計準備と他component候補は収束済み
...
- exact compatibility set / freeze status: **HOLD・未凍結**。Scratch／WireScope／Pythonのpush済みcandidateと通常dev用Minecraft runtime／Paper／Java exact baseは収束。McRemote JARは同一source／recipeのclean再buildでSHAが一致せず、再現可能なpush済みcandidateへの差替え待ち
...
- component readiness:
  - McRemote: **HOLD・artifact再現性修正待ち**。`main@6214a6a5efe5180c1cd0f374089736908b07ee34`の既知JARは191,625 bytes／SHA-256 `f293e63a77f178bc8d3cba8276e95124f2ee6b3eca77c15867a6fc5e5f166531`、同じsource／recipeからのclean再buildは同bytes／SHA-256 `a2318fd9…`。110 entriesの展開payload／CRCは一致し、差はZIP entry timestampだけ、85 tests PASSと担当報告。現行Gradle `8.14`のarchive taskがfilesystem timestampを保存し、commit＋recipeから既知output SHAを再現できないため、既知JAR／review済みhandoffをexact setへ採用しない。timestamp後加工／repackで既知SHAへ合わせず、build task自体を再現可能にした新しいpush済みcandidateへ差し替える。旧sourceと両JARは原因観測として維持するがcandidate identityではない。live未実施
...
- environment preparation: Stack PR `#26`はreview済みhead `ba9153bd9210e66a1732dfdc55cd589f567bf2ee`／merge commit `ddfb92a1dca108c906f31e670057681e0855dfef`でmerge済み。current self-verifying lockの一つの`git-build` artifactに限り、review指定SHA／lock SHA／実bytesを照合してCASへatomic create-if-absentする`mcrctl artifact import-reviewed`を実装。既存entry再hash、symlink／非regular file／容量拒否、改ざん時上書き禁止、build／download／render／apply非実行をknowledge coordinatorが差分review済み。標準port `25565`／`25575`、別portへの自動退避禁止、未知listener時のreadiness取消とbackstage照合、exact set未凍結中のhost install禁止、McRemote artifactのHTTPS／review済みgit-build二経路も一致。担当報告はrunbook回帰16 tests、全pytest、ruff、diff check PASS。GitHub CI checkは未設定。通常dev profile／runbook／review済みimport経路の準備は完了。通常dev exact baseには、b4で使用済みのMinecraft `1.21.11`、Paper `1.21.11-132`（54,846,016 bytes／SHA-256 `5ffef465eeeb5f2a3c23a24419d97c51afd7dbb4923ff42df9a3f58bba1ccfba`／credential-free HTTPS）、Java floor `21`、runtime `docker.io/itzg/minecraft-server:2026.7.2-java21`（OCI index digest `sha256:7f69fd6688e03495c8a8f5a46e8a8e82001b4465f4b55bdcd024c02c3624d8c8`）を採用する。Stack merged registryの`mcremote-paper@5`／`@6`と一致することをcoordinatorが確認済み。linux/amd64 child digest `sha256:4d9821830112c814d09d7c0a39d74f0962112fd56707f337210491ce6f8cd8f7`とruntime内Java `jdk-21.0.11+10`はpresetの追加identityにせず、実pull／起動時の照合対象とする
- input correction: McRemote作業票のknowledge SHA `f50ebb17…`は存在せず、実在する`f50ebb13f00facfc2e73163a24f002f4c8b77d43`を参照。契約差分なし
- target deployment / profile / lock: 通常dev環境`dev-integration`／`home-server@5`候補／lock未生成。physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`。先に同portで稼働していたMinecraft runtimeは停止済み。backstage PR `#3`（merge `b0dc27174860b723ca8e19360162d6de53ee57fb`）とStack PR `#26`（merge `ddfb92a1dca108c906f31e670057681e0855dfef`）は着地済みで、観測時点の未知service／listenerなし。LAN到達性とfirewallは未確認。order／lock、bootstrap contract、artifact配置、candidate deployは未実施。ケータリング型は本gate対象外
- authorized next action:
  - McRemote: archive taskへfilesystem timestampを保存しない設定と再現可能なfile orderを明示するbuild-only修正を行い、protocol／artifact version／plugin payload意味論を変更せずpushする。新commitを二つの独立したclean checkoutから同一toolchain／正準recipeでbuildし、JAR bytes／SHAが一致すること、旧`f293…`と展開entry名／payload／CRCが一致して差がarchive metadataだけであること、全決定論的testを確認する。その後、Stack templateが要求するrepository、full commit、source subdirectory、recipe／recipe SHA、toolchain／toolchain SHA、build input SHA、output filename／bytes／SHAを新candidateについて一票で返し、同一bytesを`artifact import-reviewed`へ渡せるreview済みhandoffとして準備する。既知`f293…`のtimestamp書換え／repack、tag／release／asset、artifact import、deploy、live試験は行わない
  - Scratch: source `602ecdf809f87a7e33e50d7c465b7248429e26dc`と上記artifact identityを変更せず、exact set入力組立まで待機する。candidate deploy／liveを開始しない
  - Python: source `af2b19dd4a4f0404c4bde439021ca7e017904a04`と上記artifact identityを変更せず待機する。candidate deploy／liveを開始しない
  - Stack: runtime／Paper／Java exact baseの提示を完了。McRemoteの再現可能な新candidateとreview済みgit-build provenanceが揃い、coordinatorがexact setを凍結するまで候補値を変更せず待機する。旧`f293…` bytesをimportしない。preset登録、bootstrap contract追加、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployは行わない
  - backstage: PR `#3`の修正・mergeを完了。今後も管理host上の全service／listenerの所有者・用途・期待状態をprivate inventoryと一致させ、private repoのPR／reviewは日本語を正準とする。未知service／listenerが現れた場合はenvironment readinessを取り下げて解消し、secret／private addressを公開repoへ移さない。hostへのinstallはまだ行わない
