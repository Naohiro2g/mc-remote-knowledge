...
- current phase: exact compatibility set入力組立。component決定論的candidateと通常dev環境の設計準備は収束済み
...
- exact compatibility set / freeze status: **未凍結**。plugin／Scratch／WireScope／Pythonのpush済みcandidateと、通常dev用Minecraft runtime／Paper／Java exact baseは収束。McRemote JARのreview済みhandoff／git-build provenance待ち
...
- authorized next action:
  - McRemote: source `6214a6a5efe5180c1cd0f374089736908b07ee34`をclean checkoutし、既知recipe／toolchainでJARを再生成してSHA-256 `f293e63a77f178bc8d3cba8276e95124f2ee6b3eca77c15867a6fc5e5f166531`へ一致させる。Stack templateが要求するrepository、full commit、source subdirectory、recipe／recipe SHA、toolchain／toolchain SHA、build input SHA、output filename／bytes／SHAを一票で返し、同一bytesをStackの`artifact import-reviewed`へ渡せるreview済みhandoffとして準備する。tag／release／asset、deploy、live試験は開始しない
  ...
  - Stack: runtime／Paper／Java exact baseの提示を完了。McRemoteのreview済みgit-build provenanceが揃い、coordinatorがexact setを凍結するまで候補値を変更せず待機する。preset登録、bootstrap contract追加、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployは行わない
