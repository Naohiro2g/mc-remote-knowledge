...
- authorized next action:
  ...
  - Stack: review済みhead `ba9153bd9210e66a1732dfdc55cd589f567bf2ee`のPR `#26`をrepoの通常手順でmergeし、merge commitとremote main一致を返す。merge後も、coordinatorの次許可までは実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployを行わない
