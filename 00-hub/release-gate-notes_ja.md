...
- current phase: exact compatibility set凍結済み／通常dev candidate apply・doctor許可
...
- authorized next action:
  ...
  - Stack: merged main `e3d73a5dae44086bde60f36d0fdfa3630d330b28`をm720s2へ反映し、直前preflight、validate、`resolve --allow-unverified`の`status=unchanged`、plan、CAS／render bindingの不変を確認してから、lock identity `sha256:b5840f077f0a6fd55221be1795751cad809da00a033933df3f461ca2292ab705`をexpected identityとして、正準runbookの`apply --bootstrap --yes --allow-unverified`を一回実行する。成功後に`doctor`を実行する。OCI index／linux-amd64 child digest、runtime Java、container／managed volume、Paper／McRemote mount SHA、実listener `25565`／`25575`、workstationからのLAN data-plane、token無しhelloの`auth_required`、unknown service／listenerなしを確認し、sanitized deployment確認票を返す。失敗時はその場でorder／lock／generated file／containerを手修正せず、観測とreasonを保持してcoordinatorへ戻す。製品APIのlive-auto／live-humanは行わない
  - backstage: apply後のmanaged runtime、service／listener、所有者、用途、期待状態をprivate inventoryと照合する。既存inventoryで表現済みならsanitized一致確認だけを返し、更新が必要ならhost実値を公開repoへ出さず日本語PRとして分離する。未知項目があればdeployment readinessを取り下げる
