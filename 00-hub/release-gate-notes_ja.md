...
- current phase: **protocol 22 DimensionKey全面改訂／component再固定**。Python segmentは3種eventと非破壊pollまでPASSした後、同じ実空間をPythonが入力alias`overworld`、plugin／eventがBukkit world name`world`として保持したためevent context guardがFAILして停止した。`2026-08-22-02`によりBukkit world name契約を廃止し、全surfaceをDimensionKeyへ改訂する
...
- landing verification: `2026-08-21-03`／`2026-08-21-04`の責務契約は全担当で一致。`2026-08-22-02`のdimension契約は人間批准済みで、各componentの着地確認・実装返却待ち
...
- authorized next action:
  - McRemote: 共通DimensionRef resolver、`build.setDimension`、hello／player／event／handleの`dimension`、新reason、canonical build resultを実装し、影響fixtureと再現可能JARをpushしてidentityを返す
  - Python: `setDimension()`、canonical build state同期、position／pose／event DTOとguardの`dimension`を実装し、旧wrapper／unionを残さず、Scratch common artifact受領後にwheel／sdistを再固定する
  - Scratch／WireScope: dimension command／DTO／menu自由入力、method・field・reason validatorを同時更新し、common artifactを再生成する。標準3dimensionのmenuをwire allowlistにしない
