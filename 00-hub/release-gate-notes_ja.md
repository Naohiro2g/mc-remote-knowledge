...
- current phase: exact compatibility set凍結済み／通常dev環境pre-apply readiness・runbook／inventory着地
...
- authorized next action:
  ...
  - Stack: 通常dev runbookへ、理由付きunverified acknowledgementを手書きdriftなしで設定・確認する具体的な正準操作、成功条件、失敗時の戻り先を追加し、対応するrunbook回帰testを置く。既存CLIで表現できる範囲の文書／test修正に限定し、profile、preset、bootstrap contract、order、lock、artifact、renderを変更しない。draft PR、head、差分、test結果と、上記order／lock／render identityが不変であることを返す。CLI変更が必要と判明した場合は実装せずcoordinatorへ戻す。`apply`、runtime起動、`doctor`、candidate deploy、live試験は行わない
  - backstage: draft PR `#4`をprivate repoの日本語review規律で確認し、host inventory以外を混ぜずにmergeして、reviewed head、merge commit、remote main一致だけをsanitized返却する。private inventoryの実値をknowledge／Stackへ複製しない。host状態が返却票から変わった場合はmergeより先にreadinessを取り下げる
