...
- current phase: exact compatibility set凍結済み／通常dev環境のorder・lock・artifact staging準備
...
- target deployment / profile / lock: 通常dev環境`dev-integration`／`home-server@5`／`mcremote-paper@7`／lock未生成。physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`。先に同portで稼働していたMinecraft runtimeは停止済み。backstage PR `#3`（merge `b0dc27174860b723ca8e19360162d6de53ee57fb`）、Stack PR `#26`（merge `ddfb92a1dca108c906f31e670057681e0855dfef`）、preset／bootstrap contractを登録したStack PR `#27`（merge `6451190337a9122a7f87848c49bac84f5eb82626`）は着地済み。前回観測時点の未知service／listenerなし。LAN到達性とfirewallは未確認。実order／lock、artifact staging、render、candidate deployは未実施。ケータリング型は本gate対象外
- authorized next action:
  ...
  - Stack: merged main `6451190337a9122a7f87848c49bac84f5eb82626`を使用し、`dev-integration`／`home-server@5`／`mcremote-paper@7`／`dev`／`lan-only`／`integration`／port `25565`・`25575`を実orderへ投影する。正準runbookに従い、host preflight、validate、必要な人間によるEULA承認、resolve、plan、artifact fetch、review済みMcRemote JARの`import-reviewed`、renderまでを行い、order／lock identityとdigest、CAS artifact inventory、render inventory、preflight結果をsanitized返却する。EULAが未承認なら代行せず停止する。`apply`、runtime起動、`doctor`、candidate deploy、live試験は行わない
  - backstage: m720s2の全service／listener、所有者、用途、期待状態をprivate inventoryと再照合し、未知項目なし、標準port `25565`／`25575`が使用可能、必要なoperator依存とStack checkoutの導入先がmanaged stateであることを確認する。この準備に必要なhost installは許可するが、Minecraft client／GUI／browserをserver hostへ追加せず、secret／private addressを公開repoへ移さない。未知service／listener、port競合、LAN／firewall不整合があればreadinessを取り下げ、Stackのresolve／artifact stagingより先にcoordinatorへ返す
