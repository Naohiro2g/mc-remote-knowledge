...
- current phase: exact compatibility set凍結済み／Stack preset・bootstrap contract投影のmerge前修正
...
- authorized next action:
  ...
  - Stack: draft PR `#27`の凍結済みartifact／bootstrap tupleを変えず、presetの`toolchain`を`Gradle 8.14 wrapper + Amazon Corretto JDK 21.0.10+7-LTS linux-amd64`という自己完結したidentityへ修正し、存在しない`toolchain-manifest.txt`参照を削除する。`toolchain_sha256`はreview済みsanitized manifestのdigestとして維持し、同manifestは後続のformal evidenceへ収容する。preset semantic digest／catalogを再生成し、対応testで解決不能な参照がないことを固定して全決定論的test／lint／diff check後に同draft PRの更新headを返す。coordinator再確認前にmergeせず、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployも行わない
