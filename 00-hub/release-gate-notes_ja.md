...
- exact compatibility set / freeze status: **未凍結**。plugin／Scratch／WireScope／Pythonのpush済みcandidateは収束。McRemote JARのreview済みhandoff／git-build provenanceと、通常dev用Minecraft runtime／Paper／Java exact baseの収束待ち
...
- authorized next action:
  ...
  - Stack: merged mainのregistryから、McRemote `1.21.11-2200.0.0b5`と通常dev `home-server@5`に適合するMinecraft runtime OCI、Paper JAR、Java floorのexact candidate（locator／version／bytes／digest／取得契約）をread-onlyで提示する。McRemoteのgit-build provenanceと組み合わせるpreset入力案までを返すが、preset登録、bootstrap contract追加、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployは行わない
