...
- authorized next action: McRemote `88d818…`、Python `0ba22e…`、Scratch `24077ef…`を各default branchへ統合し、merge先treeがcandidateと機能差分なしであることを確認する。その統合identityからJAR、wheel／sdist、GUI、WireScope、Bridge multi-arch OCIを生成し、digest／SBOM／provenanceと最小container／artifact smokeを固定する。公開release、tag、registry uploadは最終set返却まで行わない
