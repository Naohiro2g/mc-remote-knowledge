...
- current phase: **McRemote live-auto PASS／Python segment開始許可**。McRemoteの実機不具合修正後candidateでrunner全項目がPASSしたため、同じhost-native serverを変更せずPythonのlive-auto＋必要最小限のlive-humanへ進む
...
- exact compatibility set / freeze status: **b5 live test setをMcRemote修正版で再凍結**。既存component／policy／schemaに、host-native OpenJDK `21.0.11+10`（package `openjdk-21-jre-headless 21.0.11+10-1~24.04.2`、Java binary SHA-256 `41839c43fbb6c35f73d72f896ae2c885042fe98661128ae0d3324339f552fd53`）、Paper `1.21.11-132`／`5ffef465…ccfba`、McRemote source `fc84c8fd5e41c07c5d89671f193fdb7012eabd36`／JAR SHA-256 `7f9bf3616accc27cac100c705aa3bfc722024978a76a5505015d80047054012f`、Stack candidate `8e65214d766a4448b3fc294b794262f294c2679a`を加える。旧McRemote `ef025ce5…`／`17cdc457…aeb6`は失効し、Python／Scratch／WireScope、protocol、policy／schemaは不変。Docker OCI／container内Java／旧order・lock・renderは含めない。component、artifact bytes、Java／Paper、policy／schemaが再度変われば試験を停止してcoordinatorへ戻す
...
- authorized next action:
  - Stack: 現在のserverを変更せず維持する。segment前後の定常readiness確認は行わず、接続不能、health異常、予期しない切断が起きた場合だけ調査する。製品API試験を主導しない。PR `#29`は上記1点を直し、全回帰後にdraft解除・mergeする
  - McRemote: live-auto segment完了。追加の製品API試験、world／entity清掃、他repo開始、横断判定を行わず待機する
