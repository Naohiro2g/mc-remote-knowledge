...
- current phase: **統合後pre-OCI artifact入力＋McRemote smoke固定／最終artifact gateはOPEN**。`b6-integrated-source-set-1`からScratch／Bridge OCIを除く六artifactを再生成・durable stagingへ固定した。McRemote統合JARは通常devで起動・HEALTHY・auth否定パス・新規session認証済み代表call・同tokenの正常再起動後再利用までPASSした。Scratch test exact集計、Scratch／Bridge OCI、release判定が残る
...
- gate manifest identity: Tier 2 evidence入力は`b6-artifact-candidate-set-3`、現行sourceは`b6-integrated-source-set-1`、Scratch／Bridge OCI前の六artifactは`b6-integrated-artifact-input-1`としてそれぞれ履歴固定する。入力manifest SHA-256は`1a3b40fc3747359bd2a206f37aa4b8508989b97aedc6b6d584d8cfd49b3c4a4b`。最終`b6-artifact-candidate-set-4`はまだ成立していない。人間可読のbinding recordは`10-protocol/b6-artifact-candidate-record_ja.md` §7〜§9である
...
- component readiness:
  ...
  - Scratch: candidateをconflictなしでdevelopへno-ff mergeし、機能差分なし。isolated clean checkoutの`npm ci`＋全workspace production buildをPASSし、GUI tarは旧setと同一`1757f665…7ef5`、WireScope ZIPも同一`b3d62702…ca315`、統合SHAを持つmanifestは新digest`8570d3ee…296f`としてdurable stagingへ固定した。owner報告はprotocol 22/22、WireScope 130/130、Bridge 30/30、GUI 471件PASS＋既知1 skip。scratch-vmのowner exact集計は引き続き確認待ちで、公開、releaseは未実施
...
- authorized next action: Scratch ownerはaggregateのpassed／failed／skipped等のexact内訳を返す。現行`mc-remote-images.yml`は`push: true`でScratch／Bridgeの両GHCR packageを同時更新するため、人間承認を待つ。承認対象は統合SHA `df9264ec355dd722a848df46e96d4b0fc9340ca2`でworkflowを一回dispatchし、`ghcr.io/naohiro2g/mc-remote-scratch:sha-df9264ec355dd722a848df46e96d4b0fc9340ca2`と`ghcr.io/naohiro2g/mc-remote-bridge:sha-df9264ec355dd722a848df46e96d4b0fc9340ca2`のlinux/amd64＋linux/arm64 indexをpushする操作である。承認後は両index／platform digest、SBOM／provenance、container smokeを固定する。公開release、tag、その他registry uploadを本段階で実行しない
...
- non-claim: 観測target変更後の自動再開、live `dropped_frames`、Scratch exact aggregate、最終artifact set、Scratch／Bridge exact OCI candidate、公開release artifact freeze、b6 `GREEN`／releaseを主張しない
