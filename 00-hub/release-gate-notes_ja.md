...
- environment preparation: Stack PR `#26`はhead `0bda3cc73de63ab5466e69c2ff49e72f36250a8a`、draft解除済み、GitHub上でmergeable／clean。current self-verifying lockの一つの`git-build` artifactに限り、review指定SHA／lock SHA／実bytesを照合してCASへatomic create-if-absentする`mcrctl artifact import-reviewed`を実装。既存entry再hash、symlink／非regular file／容量拒否、改ざん時上書き禁止、build／download／render／apply非実行をknowledge coordinatorが差分review済み。担当報告はimport関連35 tests、全pytest、ruff、diff check PASS。GitHub review／CI checkは未設定。runbook／testのport訂正が残るため準備完了とはしない
- input correction: McRemote作業票のknowledge SHA `f50ebb17…`は存在せず、実在する`f50ebb13f00facfc2e73163a24f002f4c8b77d43`を参照。契約差分なし
- target deployment / profile / lock: 通常dev環境`dev-integration`／`home-server@5`候補／lock未生成。physical hostはbackstage管理下の`m720s2`、channel=`dev`、exposure=`lan-only`、purpose=`integration`、標準server portはJava `25565`／McRemote `25575`。先に同portで稼働していたMinecraft runtimeは停止済み。backstage PR `#3`は修正head `7bffaa1bc1059975375904eccae8a3e2f5b4ef18`／merge commit `b0dc27174860b723ca8e19360162d6de53ee57fb`でmerge済み。公開Stack PR `#26`の現head `0bda3cc73de63ab5466e69c2ff49e72f36250a8a`は引き続き`25566`／`25576`を使っているため、標準portへの訂正後identityを再提出する。backstage管理hostでは所有者／用途不明のservice／listenerを許容せず、inventoryと実状態を一致させる。LAN到達性とfirewallは未確認。order／lock、bootstrap contract、artifact配置、candidate deployは未実施。ケータリング型は本gate対象外
- authorized next action:
  ...
  - Stack: PR `#26`のtopology図、runbook command、host preflight説明、回帰testを標準port `25565`／`25575`へ訂正する。予定portにlistenerがある場合に別portへ自動退避せず、backstage inventoryで所有者／用途を確定して停止またはreview済みmanaged stateへ写像する規則へ直す。全test後の更新headを返し、公開review／通常mergeへ進める。`artifact import-reviewed`とexact preset／bootstrap contractの入力枠は維持するが、実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployはまだ行わない
