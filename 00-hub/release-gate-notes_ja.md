...
- current phase: **DimensionKey exact compatibility set凍結／統一live試験実行**。`2026-08-22-02`へ適合するMcRemote、Scratch／WireScope、Pythonのpush済みsourceと再現可能artifactがすべて揃い、現行host-native環境への新JAR配置とreadinessも完了した。独立McRemote runnerを重ねず、PythonとScratchの2 segmentでDimensionKey影響sliceだけを検証する
...
- authorized next action:
  ...
  - Python segment: exact wheel `370f0fef…f195`をfresh環境へ導入し、既存credentialを使い、必要な場合だけpairingする。hello／build contextが完全修飾`dimension`を返し`world`を含まないこと、`setDimension("overworld")`が`minecraft:overworld`へ正準化されること、`setBuildOrigin()`後もdimensionとoriginをserver resultから一体同期すること、`getPose()`と同一poseの`setPose()`がcanonical dimensionを返すことを確認する。`world`を旧aliasとして解釈せず`minecraft:world`として解決して`unknown_dimension`となること、`myworld:world`をclient syntaxで拒否せずserverの`unknown_dimension`まで運ぶこと、`build.setWorld`が`method_not_found`であること、各失敗後にclient contextが変わらないことを確認する。通常チャットによる`chat_posted`、block right click、projectile hitを各1件だけ発生させ、3 DTOのcanonical dimension、same-context guard成功、1件を使った意図的dimension mismatch拒否を確認する。WireScopeではhello／build／player／event frameが`dimension`を保持し`world` fieldを持たず、schema version `1`であることを確認する
...
- live-auto / live-human: **上記統一票に限り開始許可**。McRemote／Stack／backstageは試験を主導せず、server障害時だけ事実を返す
