...
- current phase: **DimensionKey exact compatibility set凍結／dev-integration JAR差し替え待ち**。`2026-08-22-02`へ適合するMcRemote、Scratch／WireScope、Pythonのpush済みsourceと再現可能artifactがすべて揃った。製品liveを再開する前に、Stackが現行host-native環境のMcRemote JARだけを新exact bytesへ差し替えてreadinessを返す
...
- authorized next action:
  - Stack／backstage: 現在のhost-native `dev-integration`へ凍結済みMcRemote JAR `f7ddbcb5…36547`だけをreview済みbytesとして配置し、一回だけ正常再起動する。配置後JAR SHA、Paper起動、McRemote version、credential domain `HEALTHY`、port `25565`／`25575`、tokenなしprotocol 22 helloの`auth_required`を確認して停止する。起動方式、service、directory、config、credential、world、runbook、backstage inventoryを変更せず、製品API runnerを開始しない
  - McRemote／Python／Scratch／WireScope: frozen candidateを変更せず待機する。追加実装、tag／release、shared環境変更、live試験を開始しない
- live-auto / live-human: **Stack配置確認まで停止**。配置確認後、coordinatorがDimensionKey影響sliceだけの統一実施票を発行し、McRemote最小wire smoke→Python→Scratchの順で再開する。既にPASSしたspawn capacity等の非影響sliceを無条件にやり直さない
