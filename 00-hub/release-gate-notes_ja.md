...
- current phase: **host-native environment READY／b5統一live試験を許可**。Stackは環境維持、各component担当は下記の割当segmentだけを実施し、knowledge coordinatorが順序・exact set・横断判定を所有する
...
- authorized next action:
  ...
  - McRemote: 最初のsegmentとしてsource `ef025ce511e12bf2e8a111e3a8c15ac561ca2be9`の更新済み`b5_live_auto.py`をexact host-native serverへ実行する。protocol 22、block／getBlocks／height／spawn、DEBUG／TRACE／FAST／flush、event poll options／cursor／loss、epoch handle、拒否時副作用なしを確認し、Minecraft内操作が必要な項目だけlive-human segmentへ明示して返す。横断判定や他repoの試験開始は行わない
...
- live-auto / live-human: **上記順序で許可**。人間参加者はMinecraft内pairing／event発火／画面確認だけを担い、実施順・判定・evidence scopeはknowledge coordinatorが所有する。各segment失敗時は後続segmentへ進まず、candidate／environmentを現場変更せず返す
