...
- current phase: exact compatibility set凍結済み／Stack preset・bootstrap contract投影
...
- authorized next action:
  ...
  - Stack: 上記server側subsetをappend-only `mcremote-paper@7`としてpreset registryへ投影し、`["home-server@5", "mcremote-paper@7", "dev", "lan-only", "integration"]`のbootstrap contractを追加する。McRemoteはexact git-build provenanceとJAR `17cdc457…`、runtime／Paperは上記accepted baseを使用し、旧`f293…`を参照しない。全決定論的test／lint／diff check後にdraft PRとして返す。この段階では実order／lock生成、host install、artifact import、render／apply／doctor、candidate deployを行わない
