...
- current phase: **DimensionKey着地確認完了／3 component実装中**。Python segmentは3種eventと非破壊pollまでPASSした後、同じ実空間をPythonが入力alias`overworld`、plugin／eventがBukkit world name`world`として保持したためevent context guardがFAILして停止した。`2026-08-22-02`によりBukkit world name契約を廃止し、全surfaceをDimensionKeyへ改訂する。McRemote、Python、Scratch／WireScopeの3担当はknowledge契約との相違なしを返却済みで、旧candidateの未追従差分だけを実装する段階へ進んだ
...
- exact compatibility set / freeze status: **失効／未凍結**。McRemote `fc84c8fd…`／JAR `7f9bf361…`、Python `af2b19dd…`／wheel `a3dacff4…`、Scratch `602ecdf8…`／common WireScope `f3ffaa1c…`は旧dimension契約のcandidateとして失効した。Paper／Java、runtime policy、observer top-level schema version `1`等の非影響identityは維持できるが、新しいcomponent commit／artifact bytesが揃うまでexact setを凍結しない
...
- authorized next action:
  - McRemote: 共通DimensionRef codec／resolver、`build.setDimension`、hello／player／event／handleの`dimension`、新reason、canonical build resultを実装し、alias／fallback／`build.setWorld`を残さない。影響fixture、runner、全決定論的test、再現可能JARを完了し、push済みsource commitとJAR filename／bytes／SHA-256を返す
  - Python: `setDimension()`、canonical build state同期、position／pose／event DTOとguardの`dimension`を実装し、旧wrapper／union／入力値の直接保存を残さない。source／fixtureの決定論的testはScratchと並行して進め、Scratch common artifact受領後にobserver artifact、wheel／sdistを再固定し、push済みidentityを返す
  - Scratch／WireScope: dimension command／DTO／menu自由入力、canonical build context、event context guard、method・field・reason validatorを同時更新し、common artifactを再生成する。標準3dimensionのmenuをwire allowlistにせず、push済みsource commit、CI、ZIP／manifestのbytes／SHA-256を返す
