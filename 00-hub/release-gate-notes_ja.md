...
- current phase: **host-native environment READY／McRemote runner認証gap修正中**。serverは変更せず、対話pairing bootstrap後に同じlive-auto本体を続行できるrunner-only差分を閉じて第1segmentを再開する
...
- exact compatibility set / freeze status: **b5 live test setを再凍結**。既存component／policy／schemaに、host-native OpenJDK `21.0.11+10`（package `openjdk-21-jre-headless 21.0.11+10-1~24.04.2`、Java binary SHA-256 `41839c43fbb6c35f73d72f896ae2c885042fe98661128ae0d3324339f552fd53`）、Paper `1.21.11-132`／`5ffef465…ccfba`、McRemote `17cdc457…aeb6`、Stack candidate `8e65214d766a4448b3fc294b794262f294c2679a`を加える。Docker OCI／container内Java／旧order・lock・renderは含めない。component、artifact bytes、Java／Paper、policy／schemaが変われば試験を停止してcoordinatorへ戻す
...
- authorized next action:
  - Stack: server artifact／Java／service／credential healthを変更せず維持し、各segment前後のreadinessと最終JAR hashを確認する。製品API試験を主導しない。PR `#29`は上記1点を直し、全回帰後にdraft解除・mergeする
  - McRemote: JARとplugin source `ef025ce511e12bf2e8a111e3a8c15ac561ca2be9`を変更せず、runner／runner testだけの後続commitを作る。`b5_live_auto.py --interactive-pair`を追加し、初回helloの`auth_required`後に`auth.pairBegin`を一度だけ実行、表示用`/mcremote pair NNN-NNN`だけを人間へ示し、同じpairing相関で有限pollする。pre-auth requestごとにfresh TCPを使い、取得tokenはmemoryだけに保持して値をlog／argv／handoffへ出さず、persistent TCPのauthenticated hello後に既存live-auto本体を続行する。非TTYでは対話待ちせずactionableに失敗する。auth-required→pairBegin→pending→token→authenticated helloとsecret非出力をmock fixtureで固定し、runner-only commitをpush後、同じhost-native serverで一回実行する。JAR SHAを再固定しない。PASS後は既定のAPI範囲を返し、人間操作が必要なevent項目だけ後続へ送る。他repo開始・横断判定は行わない
  - Python: McRemote segment PASS後、source `af2b19dd4a4f0404c4bde439021ca7e017904a04`のexact wheelをfresh venvへ入れ、同serverでb5 API、mode／flush、3 event DTO／cursor／loss、observer schema v1／compatibility v1.1、common WireScope real-browser attachを確認する。pairing等の人間操作は統一票の一部として実施し、終了時にconnection／WireScope sourceを閉じる
...
- live-auto / live-human: **McRemoteの対話pairing bootstrapと、その直後のlive-auto本体を許可**。人間参加者は表示されたpair commandをMinecraft内で一度実行するだけとし、tokenを転記しない。McRemote segment PASSまではPython／Scratch segmentへ進まない
